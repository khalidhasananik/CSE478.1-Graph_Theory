...
//     // Initialize old-to-new vertex mapping
//     int *old2new = new int[v_size];
//     g->v_size = 0;
//     g->e_size = 0;
    
//     g->edges = new Edge_t[C_size[e] + 1];  // Allocate space for edges in the subgraph
//     g->new2old = vector<int>(T_size[e]);
...
//     // Add edges to the subgraph using the new vertex indices
//     for (i = 0; i < C_size[e]; i++) {
//         int e_ = C[e][i];
//         int s = edges[e_].s;
//         int t = edges[e_].t;
...
//     // Initialize the adjacency list for DSatur
//     vector<int> *graph = new vector<int>[g->v_size];
...
//     // Arrays to hold degrees and saturation levels for DSatur
//     int *deg = new int[g->v_size];
//     int *sat = new int[g->v_size];
//     int *colors = new int[g->v_size];
//     bool *availableColors = new bool[g->v_size];

//     // Initialize DSatur data structures
//     for (i = 0; i < g->v_size; ++i) {
//         deg[i] = graph[i].size();  // Degree of each vertex
//         sat[i] = 0;                // Saturation degree of each vertex
//         colors[i] = -1;            // No vertex is colored initially
//     }

//     // DSatur Algorithm
//     for (i = 0; i < g->v_size; ++i) {
//         // Find the vertex with maximum saturation degree (and tie-breaking by degree)
//         int maxSat = -1, maxDeg = -1, selectedVertex = -1;
//         for (j = 0; j < g->v_size; ++j) {
//             if (colors[j] == -1) {  // Only consider uncolored vertices
//                 if (sat[j] > maxSat || (sat[j] == maxSat && deg[j] > maxDeg)) {
//                     maxSat = sat[j];
//                     maxDeg = deg[j];
//                     selectedVertex = j;
//                 }
...
//         fill(availableColors, availableColors + g->v_size, true);

//         // Mark colors of adjacent vertices as unavailable
//         for (int neighbor : graph[selectedVertex]) {
//             if (colors[neighbor] != -1) {
//                 availableColors[colors[neighbor]] = false;
//             }
//         }

//         // Assign the smallest available color to the selected vertex
//         int color;
//         for (color = 0; color < g->v_size; ++color) {
...
//         for (int neighbor : graph[selectedVertex]) {
//             if (colors[neighbor] == -1) {  // If it's an uncolored neighbor
...
//                     }
//                 }
//                 sat[neighbor] = neighborColors.size();  // Update saturation with the number of distinct colors
//             }
//         }
//     }

//     // Copy the colors to the graph's color array
//     g->col = new int[g->v_size];
//     for (i = 0; i < g->v_size; i++) {
//         g->col[i] = colors[i];
//     }
...
//     g->sub_v_size[k - 2] = 0;
//     for (i = 0; i < g->v_size; i++) {
//         g->sub_v[k - 2][g->sub_v_size[k - 2]++] = i;
//     }

//     // Add all edges to subgraph
//     g->sub_e_size[k - 2] = 0;
//     for (i = 0; i < g->e_size; i++) {
//         g->sub_e[k - 2][g->sub_e_size[k - 2]++] = i;
//     }
// }
...
void EBBkC_Graph_t::branch(int e, EBBkC_Graph_t* g) {
    ...
    // Initialize the adjacency list for DSatur
    vector<unordered_set<int>> graph(g->v_size);
    for (i = 0; i < g->e_size; i++) {
        int s = g->edges[i].s;
        int t = g->edges[i].t;
        graph[s].insert(t);
    ...
    // Arrays to hold degrees, saturation levels, and colors for DSatur
    int *deg = new int[g->v_size];
    int *sat = new int[g->v_size];
    int *colors = new int[g->v_size];
    bool *availableColors = new bool[g->v_size];
    ...
    // Priority queue to select vertices based on saturation and degree
    auto compare = [&sat, &deg](int a, int b) {
        if (sat[a] == sat[b]) {
            return deg[a] < deg[b];  // Tie-breaking by degree
        }
        return sat[a] < sat[b];  // Higher saturation comes first
    };
    ...
    for (i = 0; i < g->v_size; ++i) {
        // Select the vertex with maximum saturation
        int vertex;
        while (!pq.empty()) {
            vertex = pq.top();
            pq.pop();
            if (colors[vertex] == -1) break;  // Skip already colored vertices
        }
        ...
        for (int neighbor : graph[vertex]) {
            if (colors[neighbor] == -1) {  // Only update uncolored neighbors
                unordered_set<int> neighborColors;
                for (int adj : graph[neighbor]) {
                    if (colors[adj] != -1) {
                        neighborColors.insert(colors[adj]);
                ...
                // Check if the saturation degree increased
                int newSat = neighborColors.size();
                if (newSat > sat[neighbor]) {
                    sat[neighbor] = newSat;
                    pq.push(neighbor);  // Push the neighbor back into the priority queue
                }
            }
        }
    }
